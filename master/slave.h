...
typedef struct {
    // Non-category data
    uint16_t alias; /**< Configured station alias. */   // 站点别名，由ESC在上电或复位后自动读取后存入 配置站点别名寄存器(0x0012:0x0013)
...
struct ec_slave
{
    ...
    /*  Q: ring_position、station_address和effective_alias作用分别是什么，有什么区别？(!FIXME)
        ring_position  : 顺序寻址(自动增量地址)
        station_address: 设置寻址
        effective_alias: 配置站点别名（0x0012:0x0013）, 等价于slave->sii.alias
    */
    // addresses 
    ...
    // base data (ESC信息 reg0x0000 ~ reg0x00009)
    uint8_t base_type; /**< Slave type. */
    uint8_t base_revision; /**< Revision. */
    uint16_t base_build; /**< Build number. */
    uint8_t base_fmmu_count; /**< Number of supported FMMUs. */ // !FIXME: 最多16个FMMU?
